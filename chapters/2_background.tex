...
the accumulation of \textbf{technical debt} and \textbf{architectural smells},
and their formal representation through \textbf{graphs}. It then examines
...
\section{Technical Debt and Architectural Smells}
\label{sec:architectural_smells}
\label{sec:smells}

As software systems grow in size and complexity, continuous evolution,
tight deadlines, and expedient engineering decisions inevitably introduce
structural compromises. In software engineering, this accumulation of
suboptimal design choices is formalized through the metaphor of
\textbf{Technical Debt} \cite{cunningham_wycash_1992, kruchten_technical_2012}.
Much like financial debt, technical debt incurs ongoing ``interest'' in the
form of increased maintenance overhead, decreased productivity, and a higher
propensity for regressions during change operations. If left unmanaged, it
leads to progressive \textit{architectural decay} (or architectural erosion),
wherein the actual implementation of the system significantly diverges from
its intended architecture.

To identify symptoms of quality degradation, Martin Fowler popularized the
concept of a ``smell'' \cite{fowler_refactoring_2013}. A smell is not an
explicit functional error or bug; rather, it represents an observable,
surface indication of an underlying weakness in the system's design or
maintainability. In the literature, smells are broadly differentiated by their
scope and architectural granularity:

\begin{itemize}
  \item \textbf{Code Smells:} These are localized anomalies confined to
    fine-grained programming constructs, such as an individual method,
    variable, or class. Typical examples include \textit{Long Method},
    \textit{Large Class}, or \textit{Long Parameter List}. Because their
    scope is purely local, code smells can usually be detected through
    direct syntactic analysis (or Abstract Syntax Tree inspection) and can be
    remedied via standard, localized refactorings without impacting the
    overall architecture of the system.
  \item \textbf{Architectural Smells:} Unlike code smells, architectural
    smells \cite{garcia_identifying_2009, arcelli_fontana_automatic_2012}
    represent macroscopic design flaws that affect the global structure of the
    software. They arise from poorly managed dependencies, suboptimal module
    organization, or cross-cutting violations of architectural boundaries.
    Empirical investigations have demonstrated that architectural smells are
    largely independent of code-level smells
    \cite{arcelli_fontana_independent_2019}:
    a codebase may feature pristine, well-formatted classes while simultaneously
    suffering from severe architectural entanglement. Correcting an
    architectural smell requires a holistic understanding of the system and
    demands multi-component refactoring operations.
\end{itemize}

Among architectural smells, the most prominent and impactful subset consists of
\textbf{Dependency Smells}. These flaws originate specifically from improper,
excessive, or circular dependency patterns between software components (such as
cyclic couplings or architectural bottlenecks). Because dependency smells
reside in the relationships and interaction patterns between components rather
than inside localized source code blocks, they cannot be discovered through
simple line-by-line or syntactic analysis. Systematically identifying,
tracking, and mitigating these smells requires modeling the entire codebase as
a comprehensive, interconnected structure, which is achieved through
graph-based representations.

\section{Graph-Based Architectural Representation}
...
automatically detect specific Architectural Smells introduced in
Section \ref{sec:architectural_smells}, commonly designated as
\textbf{Dependency Smells} \cite{fontana_arcan_2017}. The presence of these
...
Empirical research has demonstrated that architectural smells are largely
independent of code-level smells, thus requiring dedicated structural
analysis \cite{arcelli_fontana_independent_2019}.
Notable architectural dependency smells identified in the
literature \cite{garcia_identifying_2009} and detected through graph-based
analysis tools \cite{fontana_arcan_2017} include:
...
Static analysis tools extract architectural and semantic information from
source code without executing it. This section reviews traditional
compiler-based approaches, their inherent coupling to individual languages,
and how intermediate representations mitigate this complexity.

\subsection{Compiler Front-Ends and Language Coupling}
\label{sec:compiler_frontends}
...
\subsection{Intermediate Representations and Decoupling}
\label{sec:ir_background}
...
A static analyzer relies fundamentally on parsing techniques to transform
raw source text into structured representations. This section examines
syntax tree paradigms and details the Tree-sitter framework that enables
uniform parsing in our pipeline.

\subsection{Syntax Trees: AST versus CST}
\label{sec:ast_vs_cst}
...
Constructing an accurate architectural dependency graph requires bridging
the gap between raw syntactic identifiers and their concrete semantic
declarations. This section examines the mechanisms of lexical scoping and
the role of programming language type systems in static name resolution.

\subsection{Lexical Scoping and Scope Climbing}
\label{sec:lexical_scoping}
