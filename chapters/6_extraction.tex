...
\subsubsection{Pending Attributes Mechanism}
...
\subsubsection{Dynamic Field Extraction}
...
For languages supporting explicit alias syntax (such as Rust
  \texttt{type X = Y;}
...
\begin{itemize}
  \item \textbf{Type Variable Factories}: Calls to
    \texttt{TypeVar('T', bound=...)}
...
\begin{itemize}
  ...
  \item \textbf{\texttt{split\_qualified\_name}}: Normalizes a
    ...
    parameter bracket (\texttt{<...>}) is stripped to isolate the
    nominal base path,
    while structured generic decomposition is handled beforehand by
    \texttt{extract\_type\_ref}.
\end{itemize}
...
\begin{itemize}
  \item \textbf{\texttt{extract\_identifier}}: Extracts the simple
    ...
    the \texttt{\textasciitilde} prefix for C++ destructors (e.g.,
    \texttt{\textasciitilde Server})
...
\begin{enumerate}
  ...
  \item \textbf{Generic and template types}: The extractor invokes
    ...
    \begin{itemize}
      \item Tree-sitter node kinds \texttt{template\_type} (C++),
        \texttt{generic\_type} (Java, Rust), and \texttt{subscript}
        (Python \texttt{List[T]});
      \item Textual fallback \texttt{parse\_generic\_from\_text} for bracketed
        type expressions embedded in declarator text (such as
  ...
  \item \textbf{Named fields}: The node's children are inspected for
    ...
    If found, the field's node is recursively processed with
    \texttt{extract\_type\_ref}.
...
$\text{TypeParameter}$ tuples ($\theta = \langle \iota,
\vec{\tau}_{bounds} \rangle$).
...
\begin{enumerate}
  \item \textbf{Container Discovery}: The extractor searches for type parameter
    lists under named fields (\texttt{type\_parameters},
    \texttt{parameters}) or child
    node kinds (\texttt{type\_parameter\_list},
    \texttt{template\_parameter\_list}).
    For C++, if a class or function definition is enclosed within a
    \texttt{template\_declaration} node, the template parameters are
    harvested from the
    parent container.
  \item \textbf{Parameter and Inline Bound Extraction}: Each child
    parameter node
    is decomposed by \texttt{extract\_single\_type\_parameter}. The
    parameter identifier
    $\iota$ is extracted from the \texttt{name} field or inner
    \texttt{type\_identifier}.
  ...
  \item \textbf{Trailing Constraint Clauses}: For languages that separate bounds
    ...
    \texttt{merge\_clause\_bounds} traverses the
    \texttt{where\_clause} node, matching
    each \texttt{where\_predicate} target against the extracted
    parameters and appending
  ...
  \item \textbf{Generic Base Class Extraction}: In Python, where type parameters
    ...
    \texttt{extract\_generic\_base\_parameters} inspects the
    superclass list to extract
