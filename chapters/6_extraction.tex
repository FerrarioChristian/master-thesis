...
\begin{table}[htpb]
  ...
  \begin{tabularx}{\textwidth}{|c|>{\raggedright\arraybackslash}p{4.2cm}|>{\raggedright\arraybackslash}X|}
    ...
    1 (Top) & \texttt{extraction/mod.rs} & Orchestration: Tree-sitter
...
\section{The Entry Point: \texttt{extract\_from\_cst}}
\label{sec:extract_from_cst}

The top-level extraction function \texttt{extract\_from\_cst} receives
...
\begin{enumerate}
  ...
  \item \textbf{Implementation Blocks}: Detached implementation blocks
    (e.g., Rust \texttt{impl} items).
...
Type aliases are extracted with their declared name and target type reference.
For languages supporting explicit alias syntax (such as Rust \texttt{type X = Y;}
or C++ \texttt{using X = Y;}), the parser extracts the alias name and target
using \texttt{extract\_type\_ref}.

In dynamically typed languages such as Python, type variables and aliases
are created via value assignments at module or class scope. When the
\texttt{AssignmentTypeAliases} feature flag is enabled in the language
configuration, the parser detects assignment statements where the right-hand
side denotes a type constructor:
\begin{itemize}
  \item \textbf{Type Variable Factories}: Calls to \texttt{TypeVar('T', bound=...)}
    are captured as a type alias mapping \texttt{T} to a
    $\text{TypeRef::TypeVar}(\texttt{"T"}, \vec{\tau}_{bounds})$.
  \item \textbf{Generic Subscriptions}: Subscript expressions such as
    \texttt{Vector = List[float]} are parsed as a type alias whose target is a
    $\text{TypeRef::Generic}$.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{\texttt{split\_qualified\_name}}: Normalizes a
    qualified name string (e.g., \texttt{std::vector},
    ...
    before splitting. When called on plain type text, any trailing generic
    parameter bracket (\texttt{<...>}) is stripped to isolate the nominal base path,
    while structured generic decomposition is handled beforehand by \texttt{extract\_type\_ref}.
\end{itemize}
...
\begin{itemize}
  \item \textbf{\texttt{extract\_identifier}}: Extracts the simple
    ...
    with a fallback to the \texttt{declarator} field for C and C++
    declarations. The declarator fallback recursively descends nested
    declarator nodes (\texttt{pointer\_declarator},
    \texttt{function\_declarator}, \texttt{reference\_declarator}) to
    reach the base identifier. The declarator unwrapping accepts
    both normal identifiers and \texttt{destructor\_name} nodes, preserving
    the \texttt{\textasciitilde} prefix for C++ destructors (e.g., \texttt{\textasciitilde Server})
    to distinguish them from constructors and prevent naming collisions.
...
\begin{enumerate}
  \item \textbf{Type wrapper unwrapping}: If the node is a single-child
    generic wrapper (such as a Python \texttt{type} node), extraction
    recurses directly into its inner child.
  ...
  \item \textbf{Generic and template types}: The extractor invokes
    \texttt{try\_extract\_generic} to inspect AST nodes representing
    parametric types across languages:
    \begin{itemize}
      \item Tree-sitter node kinds \texttt{template\_type} (C++),
        \texttt{generic\_type} (Java, Rust), and \texttt{subscript} (Python \texttt{List[T]});
      \item Textual fallback \texttt{parse\_generic\_from\_text} for bracketed
        type expressions embedded in declarator text (such as \texttt{std::vector<int>}).
    \end{itemize}
    The base container is extracted recursively as $\tau_{base}$, and each
    parameter is extracted as $\tau_i \in \vec{\tau}_{args}$, constructing
    $\text{TypeRef::Generic}(\tau_{base}, \vec{\tau}_{args})$.
  \item \textbf{Direct identifiers}: If the node itself is an
    \texttt{identifier}, \texttt{type\_identifier},
    \texttt{scoped\_identifier}, \texttt{scoped\_type\_identifier},
    \texttt{field\_access}, \texttt{field\_expression}, or
  ...
  \item \textbf{Named fields}: The node's children are inspected for
    ...
    If found, the field's node is recursively processed with \texttt{extract\_type\_ref}.
  ...
  \item \textbf{Textual fallback}: The node text is scanned for
    \texttt{:} or \texttt{->} characters, and the substring after the
    delimiter is parsed as a qualified name.
...
\subsection{Type Parameter and Constraint Extraction}

Generic type parameters and template variables are extracted by
\texttt{extract\_type\_parameters}, constructing an ordered vector of
$\text{TypeParameter}$ tuples ($\theta = \langle \iota, \vec{\tau}_{bounds} \rangle$).
The extraction pipeline operates in four coordinated steps:

\begin{enumerate}
  \item \textbf{Container Discovery}: The extractor searches for type parameter
    lists under named fields (\texttt{type\_parameters}, \texttt{parameters}) or child
    node kinds (\texttt{type\_parameter\_list}, \texttt{template\_parameter\_list}).
    For C++, if a class or function definition is enclosed within a
    \texttt{template\_declaration} node, the template parameters are harvested from the
    parent container.
  \item \textbf{Parameter and Inline Bound Extraction}: Each child parameter node
    is decomposed by \texttt{extract\_single\_type\_parameter}. The parameter identifier
    $\iota$ is extracted from the \texttt{name} field or inner \texttt{type\_identifier}.
    Any immediate bounds (such as Java \texttt{T extends Serializable} or Rust
    \texttt{T: Display + Clone}) are parsed into constraint type references
    $\vec{\tau}_{bounds}$.
  \item \textbf{Trailing Constraint Clauses}: For languages that separate bounds
    from the parameter list (such as Rust \texttt{where} clauses),
    \texttt{merge\_clause\_bounds} traverses the \texttt{where\_clause} node, matching
    each \texttt{where\_predicate} target against the extracted parameters and appending
    the specified trait bounds to the corresponding $\vec{\tau}_{bounds}$.
  \item \textbf{Generic Base Class Extraction}: In Python, where type parameters
    on classes are declared by inheriting from \texttt{Generic[T, U]},
    \texttt{extract\_generic\_base\_parameters} inspects the superclass list to extract
    the type variables as formal type parameters on the structured type.
\end{enumerate}
  ...
  \item \textbf{Behavioral Dependencies}: All remaining children
    ...
    \begin{itemize}
      ...
      \item \textbf{Function and Method Calls}: Invocations across
        ...
        \end{itemize}
        When callable identifiers or expressions are enclosed in protective
        parentheses (e.g., \texttt{(func)()} or macro expansions), the
        function \texttt{extract\_call\_path} recursively unwraps
        \texttt{parenthesized\_expression} CST nodes. This strips literal
        punctuation tokens and yields clean identifier sequences without
        attempting premature, error-prone semantic disambiguation during
        purely syntactic extraction.
...
During module grouping, the function \texttt{link\_out\_of\_line\_methods}
performs a universal post-extraction linking step:
