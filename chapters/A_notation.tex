...
\begin{table}[htpb]
  ...
  \begin{tabular}{|l|l|l|p{5.5cm}|}
    ...
    \textbf{Resolution Contexts} & Uppercase Greek \& Calligraphic & $\Gamma, \Delta, \Sigma$ \newline $\mathcal{E}, \mathcal{P}, \mathcal{K}, \mathcal{M}$ & Scoping environments ($\Gamma = \langle \mathcal{E}, \mathcal{P}, \mathcal{K}, \mathcal{M} \rangle$), symbol stack ($\Delta$), and lexical scopes ($\Sigma$). \\ \hline
    \textbf{Semantic Types} & Lowercase Greek & $\iota, \pi, \tau, \sigma, \delta, \theta$ & Atomic type references ($\tau$), identifiers ($\iota$), qualified paths ($\pi$), signatures ($\sigma$), imports ($\delta$), and type parameters ($\theta$). \\ \hline
...
\begin{itemize}
  ...
  \item $\tau$ (\textbf{Type Reference}): An abstract reference to a type,
    represented by a ten-state sum type modeling its discovery lifecycle:
    ...
    \texttt{Generic}($\tau_{base}, \vec{\tau}_{args}$) (parameterized type application),
    \texttt{TypeVar}($\iota, \vec{\tau}_{bounds}$) (generic type parameter placeholder),
    \texttt{Union}($\vec{\tau}$) (disjunctive type),
  ...
  \item $\theta$ (\textbf{Type Parameter}): A formal generic parameter
    $\langle \iota, \vec{\tau}_{bounds} \rangle$ declaring identifier $\iota$
    and constraint bounds $\vec{\tau}_{bounds}$.
...
\begin{itemize}
  ...
  \item $\mathcal{F}$ (\textbf{Function}): A universal functional component
    $\langle \pi, \sigma, \vec{\theta}, \mathcal{B}^?, \mathbb{B}_{ctor}, \vec{\tau}_{ann} \rangle$,
    binding a qualified name to a signature, an ordered vector of type parameters,
    an optional body block, a constructor flag, and annotations.
\end{itemize}
...
\begin{itemize}
  \item $\mathcal{S}$ (\textbf{Structured Type}): The abstraction for classes,
    ...
    $\langle \pi, k, \vec{\theta}, \vec{f}, \vec{\mathcal{F}}, \vec{\tau}_{sup}, \vec{\mathcal{S}}_{nest}, \vec{\tau}_{ann}, \vec{\delta}_{imp} \rangle$
...
\begin{itemize}
  ...
  \item $\mathcal{K}$ (\textbf{Language Configuration}): The configuration tuple
    governing language-specific semantic features (receiver keywords, transitive imports, type alias conventions).
  \item $\Gamma$ (\textbf{Execution Context}): The evaluation context
    $\Gamma = \langle \mathcal{E}, \mathcal{P}, \mathcal{K}, \mathcal{M} \rangle$ used to resolve queries, including the super-scope memoization mapping $\mathcal{M}$.
...
\begin{itemize}
  ...
  \item $\rho$ (\textbf{Name Resolution}): The language-independent phase
    ...
    \begin{itemize}
      ...
      \item $\rho_{\text{exec}}$ (\textbf{Query Execution}): Evaluates queries $q$ against the Execution Context $\Gamma = \langle \mathcal{E}, \mathcal{P}, \mathcal{K}, \mathcal{M} \rangle$.
    \end{itemize}
