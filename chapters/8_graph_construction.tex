...
\subsection{Decomposition of Compound and Generic Targets}
\label{sec:type_ref_targets}

When projecting resolved type references into directed dependency edges,
the helper function \texttt{type\_ref\_targets} decomposes compound and parameterized
variants into a sequence of concrete target endpoints $\vec{\pi}_{tgt} \subset \Pi$:

\begin{itemize}
  \item \textbf{Terminal Variants}: For $\text{Resolved}(\pi)$,
    $\text{External}(\pi)$, and $\text{Primitive}(\iota)$, the target path
    is emitted directly.
  \item \textbf{Generic Applications ($\text{Generic}(\tau_{\text{base}}, \vec{\tau}_{\text{args}})$)}:
    Both the container type $\tau_{\text{base}}$ and all specialized type arguments
    $\tau_i \in \vec{\tau}_{\text{args}}$ represent architectural dependencies.
    The decomposition collects targets from both:
    $$\text{targets}(\text{Generic}(\tau_{\text{base}}, \vec{\tau}_{\text{args}})) =
    \text{targets}(\tau_{\text{base}}) \cup \bigcup_{i=1}^k \text{targets}(\tau_i)$$
    For example, a field declared as \texttt{items: std::vector<models::User>}
    emits \texttt{UsesFieldType} edges connecting the field to both \texttt{std::vector}
    and \texttt{models::User}.
  \item \textbf{Type Variables ($\text{TypeVar}(\iota, \vec{\tau}_{\text{bounds}})$)}:
    Because a type variable is an ephemeral placeholder, its architectural couplings
    are governed by its constraint bounds:
    $$\text{targets}(\text{TypeVar}(\iota, \vec{\tau}_{\text{bounds}})) =
    \bigcup_{\tau_b \in \vec{\tau}_{\text{bounds}}} \text{targets}(\tau_b)$$
    A function parameter \texttt{x: T} with $T: \text{Display}$ emits a
    \texttt{UsesParamType} dependency toward the \texttt{Display} trait.
  \item \textbf{Union Types ($\text{Union}(\vec{\tau})$)}: The decomposition
    distributes over all alternative branches, emitting edges toward every
    reachable target in $\vec{\tau}$.
  \item \textbf{Evaluated Access Chains ($\text{EvaluatedAccess}(\tau_{\text{path}}, \tau_{\text{type}})$)}:
    Targets are collected from both the intermediate receiver path $\tau_{\text{path}}$
    and the resulting terminal type $\tau_{\text{type}}$, capturing the complete access lineage.
\end{itemize}

\subsection{Taxonomy of Dependency Edge Kinds}

The system defines 16 distinct edge kinds, organized into five
...
\begin{table}[htpb]
  ...
  \begin{tabular}{|l|l|l|l|}
    ...
    \multirow{6}{*}{\textit{Type-Level}} & \texttt{UsesFieldType} &
    ...
    & \texttt{UsesType} & StructuredType / Function & Generic constraint bound \\ \cline{2-4}
  ...
  \end{tabular}
  \caption{Complete taxonomy of the 16 dependency edge kinds,
...
\begin{itemize}
  ...
  \item \textbf{UsesType}: Generated from a structured type or function to
    each resolved constraint bound declared in its generic type parameters
    ($\vec{\theta}$). For instance, a generic entity \texttt{Handler<T: Service>}
    emits a \texttt{UsesType} edge from \texttt{Handler} to \texttt{Service},
    recording the structural dependency on the trait or interface.
