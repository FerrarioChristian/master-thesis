...
The chapter is structured into six main sections: multi-language analysis
and tooling, language-agnostic syntactic representations,
graph models for reference resolution, dependency extraction frameworks,
direct precursor prototypes developed within our research laboratory, and
finally, the architectural positioning of this thesis.
...
\subsection{Intermediate Representations and LSP}
...
\section{Dependency Extraction Frameworks}
\label{sec:program_analysis_dependency_extraction}

While the aforementioned approaches address isolated phases of analysis
(syntactic parsing or reference resolution), broader research has explored
...
discovery, whereas coarse-grained dependency analyzers model architectural
components to evaluate structural health and technical debt. Importantly,
despite their different analytical scopes, both paradigms traditionally share a
fundamental operational bottleneck: their heavy reliance on language-specific,
compiler-bound front-ends or independently maintained language grammars.
...
Depends can analyze projects written in a variety of programming languages,
including Java, C/C++, Python, Go, and Ruby.
...
grammars presents distinct architectural drawbacks compared to our approach:
\begin{itemize}
  \item \textbf{Grammar Maintenance Burden:} Because Depends relies on
    independent ANTLR grammars and custom AST visitor passes for each
    individual language, adding support for a new language or updating an
    existing one requires substantial manual engineering, reproducing the
    combinatorial front-end maintenance problem.
  \item \textbf{Parsing Algorithm Characteristics:} ANTLR utilizes a
    top-down $LL(*)$ parsing strategy, which can incur non-trivial memory and
    execution overhead on large source files compared to bottom-up GLR
    parsing algorithms.
  \item \textbf{Lack of a Formal Unified IR:} Depends maps AST nodes directly
    to dependency records without an intermediate, formal algebraic
    representation. Consequently, it cannot easily propagate generic type
    parameters, evaluate chained access expressions through unified scope
    trees, or manage complex module partitioning without custom,
