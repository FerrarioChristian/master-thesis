...
\subsection{Resolution of Disjunctive Types: \texttt{Union}}
\label{sec:union_resolution}

In heterogeneous, multi-language codebases, type references frequently
exhibit disjunctive or polymorphic characteristics. To faithfully capture
...
Such variants originate during syntactic extraction (Phase 1) in three
primary scenarios:
\begin{enumerate}
  ...
  \item \textbf{Generic Type Arguments}: For parameterized data types
    (e.g., \texttt{Result<T, E>} or \texttt{Map<K, V>}), the type extraction
    heuristics package multiple type parameters into a union reference so that
    every associated domain entity participates in architectural dependency
    analysis.
...
by polymorphic or generic usage are faithfully represented in the architectural
dependency graph.
...
\begin{table}[htpb]
  ...
  \begin{tabular}{|l|c|c|c|c|c|}
    ...
    \texttt{support\_impl\_blocks} & & \checkmark & & & \\ \hline
