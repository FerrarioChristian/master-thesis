...
\end{align*}
Each flag controls whether a specific grouping strategy is active
during extraction. Java defines module namespaces via package
declarations (\texttt{PackageDeclBased}); Rust and Python infer
hierarchies from the filesystem (\texttt{FileBased}, \texttt{DirectoryBased}),
with Rust additionally supporting explicit inline modules
(\texttt{InlineModBased});
C++ relies primarily on explicit namespace blocks (\texttt{NamespaceBased});
and C exhibits no module system, placing all extracted components in a flat
root module when all flags are false. Table \ref{tab:lang_config_modules}
...
Languages like Rust separate the definition of data structures (\texttt{struct})
from their behavior (\texttt{impl} blocks), allowing multiple blocks to extend
the same type across different modules. Similarly, C++ supports out-of-line
method definitions. The analyzer extracts these as temporary implementation
extensions $\mathcal{X}$, later flattened into the target type during
resolution.

\paragraph{Forward Declarations (\texttt{ForwardDeclarations})} The classical
C and C++ translation unit model separates declarations in header files
(\texttt{.h}/\texttt{.hpp}) from definitions in source files
(\texttt{.c}/\texttt{.cpp}).
Enabling this flag activates an out-of-line method linking step that pairs
orphan method implementations with their enclosing class declarations.
