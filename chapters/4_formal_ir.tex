...
\begin{align*}
  ...
  &\mid \text{Generic}(\tau_{base}, \vec{\tau}_{args}) \\
  &\mid \text{TypeVar}(\iota, \vec{\tau}_{bounds}) \\
...
\begin{itemize}
  ...
  \item \textbf{Generic($\tau_{base}, \vec{\tau}_{args}$)}: Represents a
    parameterized or template type application (such as \texttt{Vector<T>} or
    \texttt{Map<K, V>}). It couples a base type reference $\tau_{base}$
    with an ordered sequence of argument type references $\vec{\tau}_{args}$.
  \item \textbf{TypeVar($\iota, \vec{\tau}_{bounds}$)}: Represents a
    generic type parameter or placeholder variable identified by $\iota \in \mathcal{I}$,
    constrained by an ordered sequence of upper bounds $\vec{\tau}_{bounds}$.
...
We define the \textbf{Type Parameter} construct ($\theta$) to model generic
type variables and template parameters declared on structured types or functions:
$$\theta = \langle \iota, \vec{\tau}_{bounds} \rangle$$
where $\iota \in \mathcal{I}$ is the parameter identifier and $\vec{\tau}_{bounds}$
is an ordered vector of type references specifying constraint bounds (such as interface
implementations, trait bounds, or base class restrictions).
...
variables are formalized as a vector of $f$. The block
...
specific signature, an ordered vector of generic type parameters,
an optional root block modeling its body, a boolean flag for
constructors, and a vector of annotations:
$$\mathcal{F} = \langle \pi, \sigma, \vec{\theta}, \mathcal{B}^?,
...
$$\mathcal{S} = \langle \pi, k \in \text{Kind}, \vec{\theta}, \vec{f},
...
\begin{itemize}
  ...
  \item $\vec{\theta}$: the vector of generic type parameters declared on the entity.
...
For a parameterized entity, such as \texttt{class Repository<T, ID extends Serializable>}, the type parameters are explicitly preserved:
$$\vec{\theta} = [ \langle \texttt{T}, [\ ] \rangle, \langle \texttt{ID}, [\tau_{\texttt{Serializable}}] \rangle ]$$
...
\begin{align*}
  ...
  &\ \text{AssignmentTypeAliases}, \\
  &\ \text{SelfKeyword}, \\
  &\ \text{SelfTypeKeyword}, \\
  &\ \text{ImplicitFirstParamAsSelf}, \\
  &\ \text{ExtractDynamicFields} \rangle
\end{align*}
...
\paragraph{Assignment Type Aliases (\texttt{AssignmentTypeAliases})}
In dynamically typed or scripting languages such as Python, type variables and
type aliases are introduced through assignment expressions at module or class scope
(for instance, \texttt{T = TypeVar('T', bound=Entity)} or \texttt{Vector = list[float]}).
Enabling this flag activates parser heuristics that recognize type variable
instantiations and subscripted type annotations on assignment targets, promoting them
into formal type aliases ($\mathcal{A}$) or type parameters ($\theta$) instead of
discarding them as runtime variable assignments.

\paragraph{Universal Handling of Detached Definitions}
Multi-language analysis frameworks frequently rely on ad-hoc configuration
flags to gate the extraction of detached implementation blocks (such as Rust \texttt{impl} blocks)
or out-of-line method declarations (such as C++ qualified definitions \texttt{ClassName::method}).
In OmniDeps, these constructs are treated as universal cases within the Intermediate Representation:
\begin{itemize}
  \item \textbf{Implementation Blocks ($\mathcal{X}$)}: When the concrete syntax tree contains
    detached implementation containers (\texttt{impl\_item} in Rust), the extractor emits an
    $\mathcal{X}$ tuple. For languages where methods are strictly declared within the class body
    (Java, Python), such syntax nodes do not exist in the grammar, naturally yielding $\vec{\mathcal{X}} = \emptyset$.
  \item \textbf{Out-of-Line Method Linking}: Function definitions exhibiting multi-part qualified names
    (such as \texttt{void MyClass::myMethod()} in C++) are recognized during module grouping and linked
    to their enclosing structured type (or converted into a synthetic $\mathcal{X}$ block). Languages
    lacking qualified top-level function declarations (C, Java, Python, Rust) naturally yield no qualified
    free functions, rendering the grouping step a neutral no-op.
\end{itemize}
Because both mechanisms are syntactically self-discriminating, they operate uniformly across all
languages without requiring dedicated flags in $\mathcal{K}$, preserving configuration parameters
strictly for semantic behaviors that cannot be inferred from local syntax alone.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{|l|c|c|}
    \hline
    \textbf{Language} & \textbf{TransitiveImports} & \textbf{AssignmentAliases} \\ \hline
    \textbf{Java}     & False & False \\ \hline
    \textbf{Rust}     & False & False \\ \hline
    \textbf{Python}   & True  & True  \\ \hline
    \textbf{C++}      & False & False \\ \hline
    \textbf{C}        & False & False \\ \hline
  \end{tabular}
...
\paragraph{Type Self-Reference (\texttt{SelfTypeKeyword})}
Rust introduces the \texttt{Self} keyword to reference the implementing type
within an \texttt{impl} block, injected as a type alias into the local scope.

\paragraph{Structural Deref Delegation}
In languages featuring smart pointer dereferencing through traits (such as Rust's
\texttt{std::ops::Deref}), method and member accesses on a container implicitly
delegate to an inner target type. In OmniDeps, when an implementation block
implements a trait named \texttt{Deref} and defines an associated type alias
\texttt{type Target = T}, the target type $T$ is appended directly to the implementing
type's super-types in the Scope Tree. This structural rule integrates seamlessly
with the standard Method Resolution Order without requiring an ad-hoc configuration parameter in $\mathcal{K}$.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{|l|c|c|c|c|}
    ...
    & \textbf{DynamicFields} & \textbf{SelfType} \\ \hline
