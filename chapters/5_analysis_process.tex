...
blocks (C++), or a flat fallback (C). In the presence of package
markers or source files lacking top-level declarations (such as empty
\texttt{\_\_init\_\_.py} files or comment-only headers), extraction
yields $\vec{\mathcal{M}} = \emptyset$ without allocating extraneous
nodes or triggering indexing faults. For C++, an additional
...
raw symbolic identifiers $\text{Unresolved}(\pi_{\text{raw}})$,
parameterized type applications $\text{Generic}(\tau_{base}, \vec{\tau}_{args})$,
type parameter variables $\text{TypeVar}(\iota, \vec{\tau}_{bounds})$,
or composite
...
raw identifier and deferred query expression into one of eight post-resolution
...
\begin{itemize}
  ...
  \item $\text{Generic}(\tau_{\text{base}}, \vec{\tau}_{\text{args}})$: A
    parameterized type application whose base type reference $\tau_{\text{base}}$
    and type argument references $\vec{\tau}_{\text{args}}$ have each been
    recursively resolved.
  \item $\text{TypeVar}(\iota, \vec{\tau}_{\text{bounds}})$: A generic
    type parameter or placeholder variable whose bound references
    $\vec{\tau}_{\text{bounds}}$ have been evaluated against visible scopes.
...
\begin{enumerate}
  ...
  \item \textbf{Edge Generation}: The module tree is traversed a
    ...
    directed edges. The system defines 16 distinct edge kinds
    organized into five categories: structural (containment and
    nesting), inheritance, type-level (field types, parameter types,
    return types, generic bounds), behavioral (calls, instantiations, field
